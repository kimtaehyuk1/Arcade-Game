...
#화면 타이틀 설정
pygame.display.set_caption("F학점피하기") #게임이름
...
background = pygame.image.load("C:/Users/98tae/OneDrive/바탕 화면/PythonWorkspace/Arcade-Game/pygame_basic/게임배경(찐).png") #load(여기다 경로 주면 이 경로 파일 불러옴)
#탈출문자 어쩌구 로인해 \를 / 로 바꾸기(경로는 파일 만들어진거 copy path로 가져옴)


#캐릭터(스프라이트) 불러오기
character = pygame.image.load("C:/Users/98tae/OneDrive/바탕 화면/PythonWorkspace/Arcade-Game/pygame_basic/내얼굴캐릭터(찐).png")
...
# 피할 학점 만들기
enemy = pygame.image.load("C:/Users/98tae/OneDrive/바탕 화면/PythonWorkspace/Arcade-Game/pygame_basic/F학점(찐).png")
enemy_size = enemy.get_rect().size #이렇게 적으면 캐릭터 가로 세로 가 얼만지 구해옴
enemy_width = enemy_size[0]
enemy_height = enemy_size[1]
enemy_x_pos = random.randint(0, screen_width - enemy_width) #학점이 랜덤으로 떨어지게 끔
enemy_y_pos = 0 #화면 세로크기 가장 아래에(세로)
enemy_speed = 10
...
while running:
    ...
    #루프 돌면서 학점 위치가 위에서 떨어져야 된다.
    enemy_y_pos += enemy_speed

    if enemy_y_pos > screen_height: #y는 +가 밑에기 때문에 이소리는 즉슨 화면 밑바닥 떨어졌으면
        enemy_y_pos = 0
    ...
    screen.blit(enemy, (enemy_x_pos, enemy_y_pos))  #학점 그리기
